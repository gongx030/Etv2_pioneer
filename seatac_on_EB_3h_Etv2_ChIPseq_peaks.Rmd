...
par(mfrow = c(1, 2))
k <- 2
gr$cluster <- kmeans(cbind(h, v), k)$cluster
...
# Write the estimated mono nucleosome / NFR ratio as a BigWig file
This file will be used by ``get_normalizeToMatrix`` to generate the heatmap
```{r}
bw_file <- '/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_mono_nucleosome_vs_nfr_seatac.bw'
gr_bin <- unlist(tile(gr, width = bin_size))
gr_bin$score <- log(gr$mono_nucleosome_reads + 1) - log(gr$nfr_reads + 1)
seqlevels(gr_bin) <- seqlevels(BSgenome.Mmusculus.UCSC.mm10)
seqlengths(gr_bin) <- seqlengths(BSgenome.Mmusculus.UCSC.mm10)
seqinfo(gr_bin) <- seqinfo(BSgenome.Mmusculus.UCSC.mm10)
cvg <- GenomicRanges::coverage(gr_bin, weight = 'score')
win <- unlist(tile(reduce(gr_bin), width = bin_size))
win$score <- mean(cvg[win])
seqlengths(win) <- seqlengths(BSgenome.Mmusculus.UCSC.mm10)
export(win, con = bw_file, format = 'BigWig')
```
...
bname <- 'EB_NoDox_D25_nfr'
base.name <- sprintf('%s/%s', dataset_dir(dataset), bname); treatment_files <- '/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_nfr.bam'
...
bname <- 'EB_NoDox_D25_mono_nucleosome'
base.name <- sprintf('%s/%s', dataset_dir(dataset), bname); treatment_files <- '/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_mono_nucleosome.bam'
macs2.callpeak(treatment_files, base.name, format = 'BAMPE', genome = genome, broad = FALSE, qvalue.cutoff = 0.05, fold.change = FALSE, update = TRUE, call.summits = TRUE, shift = -100, extsize = 200)
...
```{r}
gs <- 'EB_Dox_3h_Etv2'; extend <- 1000; w <- 50; smooth <- TRUE; target_ratio <- 0.2; mc.cores <- 4
bw_files <- c(
  ...
  'EB_NoDox_D25' =    '/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_treat_pileup.bw',
	...
	'EB_NoDox_D25_nfr' = 	'/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_nfr_treat_pileup.bw',
	'EB_NoDox_D25_mono' = 	'/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_mono_nucleosome_treat_pileup.bw',
...
extend <- window_size / 2; w <- 5; smooth <- FALSE; target_ratio <- 1; mc.cores <- 4
bw_files2 <- c(
	'EB_NoDox_D25_mono_vs_nfr_seatac' = 	'/panfs/roc/scratch/gongx030/datasets/dataset=Etv2ATAC_version=20190228b/EB_NoDox_D25_mono_nucleosome_vs_nfr_seatac.bw'
)
n2m_files2 <- get_normalizeToMatrix(gr, gs, bw_files2, extend = window_size, w = w, smooth = smooth, target_ratio = target_ratio, mc.cores = mc.cores, force = FALSE)


n2m_files <- c(n2m_files, n2m_files2)
...
col_fun[['EB_NoDox_D25_mono_vs_nfr_seatac']] <- colorRamp2(quantile(mat[['EB_NoDox_D25_mono_vs_nfr_seatac']], c(0.005, 0.5, 0.995)), c('blue', 'black', 'yellow'))

i <- 1:length(gr)
sp <- factor(gr$cluster)
ta <- HeatmapAnnotation(enriched = anno_enriched(gp = gpar(lty = 1, lwd = 2, col = 1:nlevels(sp)), axis_param = list(facing = 'inside', at = -1000)))
h <- EnrichedHeatmap(mat[['EB_NoDox_D25_mono_vs_nfr_seatac']][i, ], col = col_fun[['EB_NoDox_D25_mono_vs_nfr_seatac']], split = sp[i], name = 'EB_NoDox_D25_mono_vs_nfr_seatac', top_annotation = ta, pos_line = FALSE)
ss <- c('EB_NoDox_D25_mono_vs_nfr', 'EB_Dox_3h_Etv2', 'EB_Dox_12h_Etv2')
