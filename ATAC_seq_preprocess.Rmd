...
### Calling peaks by Macs2 (generating jobs)
```{r}
...
gr <- gr[!gr %over% blacklist]	# remove the ATAC-seq peaks overlapping with blacklist
...
### [2020-08-19] Generating a read counts matrix for each ATAC-seq sample, over a union peak set across all ATAC-seq samples
* These data will be used to perform a sample level chromVAR analysis
...
# check if the pileup bigwig file exist
bw_files <- gsub('_summits.bed', '_treat_pileup.bw', bed_files)
...
se <- SummarizedExperiment(assays = SimpleList(counts = X), rowRanges = peaks, colData = data.frame(bed_file = bed_files, bw_file = bw_files, group = names(bw_files)))
